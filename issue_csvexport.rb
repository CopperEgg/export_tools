...
def issues_to_csv(issues_json)
  begin
    ...
    $file_name = "#{$output_path}/issues_#{$t_start.to_s.gsub(':','_')}_#{$t_end.to_s.gsub(':','_')}.csv"
    headers = ['ID','State','Short Message','Additional Information','Type','Created At','Updated At',
      'Cleared At','Notified At', 'Ignore Until', 'Annotation Id','Group Def Id']
...
$t_start = Time.new(options.start_year, options.start_month, options.start_day,
  options.start_hour, options.start_min, options.start_sec)
$t_end = Time.new(options.end_year, options.end_month, options.end_day,
